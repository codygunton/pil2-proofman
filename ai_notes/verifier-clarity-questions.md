...
>> Agreed. Will remove the alias and just use the underlying type directly.
...
>> Will rename to something like `FRIQueryIdx` or `ExtendedDomainIdx` depending on what it actually indexes into.
...
>> Good point. This is part of a larger Merkle tree abstraction task—the verifier shouldn't be thinking about sponge widths at all. Could have `MerkleTree4` and `MerkleTree16` classes (or similar) that encapsulate these details.
...
>> Will investigate whether these can be eliminated entirely. The protocol has named stages (trace, quotient, evals, FRI); we should be able to refer to them by name rather than computing `n_stages + SOME_OFFSET`.
...
>> Will trace this back to the C++ and add a comment explaining the derivation, or compute it from Poseidon2 parameters if that's what it is.
...
>> Will investigate galois library capabilities. At minimum, a `ff3_from_interleaved(buffer, offset)` helper would eliminate the repeated 2,1,0 pattern. If galois has native support for coefficient ordering, even better.
...
>> Will trace back to where these nested lists originate (likely proof deserialization) and flatten them there so downstream code doesn't need to unwrap.
...
>> Will replace all inline challenge slicing with `_get_challenge` calls. Low-risk, high-consistency improvement.
...
>> Will rename. This likely requires changes in stark_info.py where EvMap is defined, plus all usages.
...
>> Will investigate what the actual semantic distinction is and propose clearer names. May need to check C++ or pil2-proofman-js for context.
...
>> Will determine the meaning and either expand the abbreviation or add a comment. Likely "committed" (as in committed polynomials).
...
>> Will check C++. If x_div_x_sub is the established name in the codebase, we keep it for consistency with C++. If not, will propose a clearer name.
...
>> Will rename to something like `fri_poly_evals_at_queries` or similar descriptive name.
...
>> Agreed, the function structure is fine. The transcript labeling idea is interesting for debugging—could add an optional `label` parameter to `put()` that's stored for later printing. Low priority but nice-to-have. 
...
>> Yes, this connects to the type system cleanup (#29). Once we eliminate field triple gymnastics and use FF3 consistently, this function should become more readable. The ev_map documentation is separate but related.
...
>> This is a key question. The prover needs buffers for performance with NTT/Merkle ops. The verifier processes single queries—it may not need buffers at all. Investigating this could lead to a much cleaner verifier that works directly with named polynomial evaluations. Flagging as potential big win but separate project.
...
>> Possibly. A dict keyed by `(name, index)` could be clearer than linear search IF the key structure is obvious. Need to prototype and compare. The current "count same-name entries" logic is definitely confusing.
...
>> Will fix the silent failure—should raise an exception if the challenge isn't found. The direct indexing question connects to the broader map structure issue (#18).
...
>> Will prototype both approaches and compare. Pre-grouping might actually be clearer if the grouped structure has good names, e.g., `challenges_by_stage[stage]`. But if it obscures the data flow, we skip it.
...
>> Acknowledged, will leave as-is. Could add a brief comment noting it's for recursion support.
...
>> Will investigate what `last_level_verification` actually does. If it's purely an engineering optimization (caching intermediate Merkle levels), it should be hidden inside the Merkle tree abstraction or removed from the spec entirely.
...
>> Will fix. Late imports usually indicate the module boundary is in the wrong place. May require moving some code or introducing an interface.
...
>> Will investigate. Separating parsing from verification could make the verification logic cleaner and more auditable. The parsing phase would produce a well-typed intermediate structure.
...
>> Will add context to error messages: query index, step number, expected vs actual values where applicable.
...
>> Acknowledged, will keep the current bool+print pattern.
...
>> This is a core issue. Will document the type discipline clearly and enforce it: FF and FF3 from galois are already numpy-compatible, so we should work with them directly rather than converting to/from int/list. May need to add this to CLAUDE.md or a style guide that agents will follow.
...
>> Will add documentation for STARKProof structure—either as docstrings or a diagram. The proof structure should be understandable without tracing through deserialization code.
...
>> Will clarify the distinction with documentation or consolidation. Likely: `stark_struct` is the core protocol parameters while `stark_info` adds AIR-specific metadata. If the distinction isn't meaningful, consider flattening.
...
>> Acknowledged, out of scope for this work.

---

**See also:** [verifier-simplification-meta-plan.md](./verifier-simplification-meta-plan.md) for the project plan to address these issues.
