...
| File | Description |
|------|-------------|
| `test_fri.py` | Pytest suite validating that Python FRI outputs match C++ golden values byte-for-byte. Tests hash functions, query indices, FRI folding, and end-to-end prove(). |
| `test_vectors.py` | Loader module for golden test vectors. Loads data from JSON files in test-data/. |
| `test-data/simple-left.json` | Golden test vectors for SimpleLeft AIR (small, 8-row test case). |
| `test-data/lookup2-12.json` | Golden test vectors for Lookup2_12 AIR (large, 4096-row test case with FRI folding). |
...
| `pyproject.toml` | Python project configuration with dependencies (galois fork, numpy, pytest). |
...
```bash
python -m pytest executable-spec/test_fri.py -v
```
