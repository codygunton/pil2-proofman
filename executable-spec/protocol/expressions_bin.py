    ...
    def _load_expressions_bin(self, reader: BinFileReader):
        ...
        for h in range(n_hints):
            ...
            for f in range(n_fields):
                ...
                hint_field.name = reader.read_string()

                n_values = reader.read_u32_le()

                for v in range(n_values):
                    hint_field_value = HintFieldValue()

                    operand_str = reader.read_string()
                    hint_field_value.operand = optype_from_string(operand_str)

                    if hint_field_value.operand == OpType.number:
                        hint_field_value.value = reader.read_u64_le()
                    elif hint_field_value.operand == OpType.string_:
                        hint_field_value.string_value = reader.read_string()
                    else:
                        hint_field_value.id = reader.read_u32_le()

                    if hint_field_value.operand in [OpType.custom, OpType.const_, OpType.cm]:
                        hint_field_value.row_offset_index = reader.read_u32_le()

                    if hint_field_value.operand == OpType.tmp:
                        hint_field_value.dim = reader.read_u32_le()

                    if hint_field_value.operand == OpType.custom:
                        hint_field_value.commit_id = reader.read_u32_le()

                    n_pos = reader.read_u32_le()
                    for p in range(n_pos):
                        pos = reader.read_u32_le()
                        hint_field_value.pos.append(pos)

                    hint_field.values.append(hint_field_value)

                hint.fields.append(hint_field)
            ...
            for f in range(n_fields):
                ...
                for v in range(n_values):
                    ...
                    if hint_field_value.operand == OpType.number:
                        ...
                        hint_field_value.string_value = reader.read_string()
                    elif hint_field_value.operand in [OpType.airgroupvalue, OpType.airvalue]:
                        hint_field_value.dim = reader.read_u32_le()
                        hint_field_value.id = reader.read_u32_le()
                    elif hint_field_value.operand in [OpType.tmp, OpType.public_, OpType.proofvalue]:
                        hint_field_value.id = reader.read_u32_le()
                    else:
