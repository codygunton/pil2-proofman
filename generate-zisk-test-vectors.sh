...
#   2. Runs cargo-zisk prove to generate JSON proofs + publics
...
#
# Prerequisites:
#   - A built cargo-zisk binary (from the zisk repo)
#   - A Zisk proving key (generated by pil2-proofman-js setup)
#   - A Zisk guest ELF (built with cargo-zisk sdk)
#   - LD_LIBRARY_PATH set for Rust stdlib and Intel OneAPI libs
#   - Python environment set up: cd executable-spec && ./setup.sh
#
# Environment:
#   ZISK_DIR        Path to zisk repo with built cargo-zisk (default: ../zisk)
...
# --- Defaults ---
ZISK_DIR="${ZISK_DIR:-$(cd "$ROOT_DIR/../zisk" 2>/dev/null && pwd)}"
