...
The Add256 AIR ({src}`zisk/precompiles/big_int/src/add256.rs#Add256SM`, $2^{20}$ rows) provides 256-bit unsigned addition.
Used as a building block for multi-precision arithmetic.

(sec:aritheq)=
## ArithEq

The ArithEq AIR ({src}`zisk/precompiles/arith_eq/src/arith_eq.rs#ArithEqSM`, $2^{20}$ rows) provides 256-bit modular arithmetic
...
The ArithEq384 AIR ({src}`zisk/precompiles/arith_eq_384/src/arith_eq_384.rs#ArithEq384SM`, $2^{20}$ rows) extends ArithEq to 384-bit fields,
supporting BLS12-381 elliptic curve operations.

(sec:keccakf)=
## Keccakf

The Keccakf AIR ({src}`zisk/precompiles/keccakf/src/keccakf.rs#KeccakfSM`, $2^{17}$ rows) implements the Keccak-f[1600] permutation,
...
The Sha256f AIR ({src}`zisk/precompiles/sha256f/src/sha256f.rs#Sha256fSM`, $2^{18}$ rows) implements the SHA-256 compression function.
